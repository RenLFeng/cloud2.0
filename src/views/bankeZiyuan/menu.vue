<template>
  <div class="mulu-wrap">
    ...
    <div class="foot-btn position-fb clearfix">
      <p class="fl new">
        <mt-button @click="newFolder">{{$mt('新建文件夹')}}</mt-button>
      </p>
      <p class="fr keep">
        <mt-button @click="submit" :disabled="canSubmit">{{$mt('确定')}}</mt-button>
        <!-- :disabled="canSubmit" -->
      </p>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    showPrve() {
      if (this.repositoryopen) {
        if (this.curRoots.length > 1 || this.tempmenuData.length) {
          return true;
        }
        return false;
      } else {
  ...
  methods: {
    ...
    newFolder() {
      ...
      if (this.cfrom) {
        url += "&banketype=" + 1;
      } else if (this.repositoryopen) {
        let parentid = this.Repositoryclickdata.parentid;
        let banketype = this.Repositoryclickdata.banketype;
        url =
          "/api/bankefile/menuadd?parentid=" +
          parentid +
          "&banketype=" +
