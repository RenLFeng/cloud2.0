<template>
  <div class="bind-school-wrap">
    <mt-header :title="$mt('学校绑定')" class="mint-header-f">
      <mt-button slot="left" icon="back" @click="back">{{$mt('返回')}}</mt-button>
    </mt-header>
    <div class="main">
      <mt-cell :title="$mt('选择学校')" is-link @click.native="bindSchool" class="border-b">{{schoolname}}</mt-cell>
      <mt-field :label="`${$mt('工号')}/${$mt('学号')}`" :placeholder="$mt('请输入工号或学号')" v-model="sno" class="border-b"></mt-field>
      <mt-field :label="$mt('校验码')" :placeholder="`${$mt('请输入校验码')}，${$mt('没有则不填')}`" v-model="schoolpwd"></mt-field>
      <div class="button-worp">
        <mt-button class="button-auto-87" @click="onbindSchool">{{$mt('绑定')}}</mt-button>
      </div>
    </div>
      ...
      this.$http
        ...
        .then(res => {
          if (res.data.code == "0") {
            ...
            let tipmsg = this.$mt('绑定失败') + res.data.msg;
            if (res.data.msg == "has bind in school") {
              tipmsg = this.$mt('已绑定到该学校');
            } else if (res.data.msg == "invalid password") {
              tipmsg = this.$mt('校验码不匹配');
            }
            Toast(tipmsg);
          }
          Indicator.close();
