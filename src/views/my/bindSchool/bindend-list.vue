<template>
  <div class="bindend-list-wrap">
    <mt-header :title="$mt('已绑定的学校')" class="mint-header-f">
      <mt-button slot="left" icon="back" @click="goback">{{$mt('返回')}}</mt-button>
    </mt-header>
    <div class="main main-f">
      <p class="title fontsmall" v-if="BindSchoolListData.length">{{$mt('已绑定的学校')}}</p>
    ...
    <div class="button-worp position-fb" v-if="!detail_go_school">
      <mt-button class="button-auto-87" @click="onbindNewSchool">{{$mt('绑定新学校')}}</mt-button>
    </div>
    <mt-popup
