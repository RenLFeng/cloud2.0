  ...
  <div class="pingce-dtail-warp">
    <div class="main">
      ...
      <div class="big-wrap">
        <Vanbr
          :tabBar="tabBar"
          :width="pingceItemfile.ptype>3?'20':18"
          @selectClick="onselectClick"
          seleid="label"
          v-if="pingceItemfile.ptype!='10' && memberData.length"
        />
        ...
        <!--  学生提交展示 -->
        <div
          class="list-main overflow-scroll"
          v-if="memberData.length"
          :class="pingceItemfile.ptype!=10?'top':''"
        >
          <div class="content">
        ...
        <div v-else class="list-main">
          <span v-if="pingceItemfile.ptype!=10" class="position-c">{{$mt('无提交')}}</span>
        </div>
      </div>
    </div>
    <mt-popup
    ...
    >
      <mt-header :title="`${memBerItem.name} ${$mt('答案')}`">
...
<style>
.pingce-dtail-warp .van-navbr-wrap {
  position: absolute;
  top: -1px;
  left: 0;
  margin-top: 0;
