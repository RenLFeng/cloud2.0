<template>
  <div class="pingceing-warp">
    <mt-header v-if="hasnavbar" :title="$mt(pagetitle)">
      <mt-button icon="back" slot="left" @click="onBack">{{$mt('返回')}}</mt-button>
    </mt-header>
    ...
    <div class="main">
      ...
      <div class="pingce-ing" v-if="isPingce">
        ...
        <div class="footer">
          <p class="tit border-b" v-if="pingceData.ptype!='10'">{{$mt('请点击选项作答')}}</p>
          <Duoxuan
            @submitFn="onSubmit"
...
export default {
  ...
  computed: {
    ...
    pagetitle() {
      if (this.isPingce) {
        if (this.pingceData.ptype=='6'&&!this.pingceData.files) {
          return '快速抢答';
        }
      ...
      this.$http
        ...
        .then(res => {
          if (res.data.code == "0") {
            ...
            //     files:
            //       "/downloads/bankefile/20201022/bd0668f6dcc29a41f794e01cb058a5d0.jpg",
            //     id: 1143,
            //     info: null,
            //     joinnum: 0,
            //     optdesc: '{\n  "opts" : ["A","B"]\n}',
      ...
      this.$http
        ...
        .then(res => {
          if (res.data.code == "0") {
            ...
            this.tempAnswer = [];
            Toast(this.$mt("提交失败") + ":" + res.data.msg);
          }
        })
        .catch(err => {
          this.tempAnswer = [];
