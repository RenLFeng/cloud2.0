  ...
  <div class="footer-item footer-item-j">
    <ul class="clearfix" v-if="type=='1'">
      <li class="fl" v-for="(item,index) in indexNumber" :key="index" @click="select(item)">
        <span :class="item.isTrue?'act':''">{{$mt(item.v)}}</span>
      </li>
    </ul>
    <div v-if="type=='5'">
      <mt-field
        ...
        :placeholder="$mt('请输入答案后点击提交')"
        type="textarea"
        rows="6"
      ></mt-field>
    </div>
    <div v-if="type=='6'">
      <div class="Responder tc fontnormal" v-if="!ponderState" @click="ResponderFn">{{$mt('点击抢答')}}</div>
      <p v-if="ponderState" class="colory tc Responder-ed">{{$mt(ResponderText)}}</p>
    </div>
