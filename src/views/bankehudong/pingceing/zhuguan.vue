<template>
  <div class="footer-item-z">
    ...
    <ul class="clearfix">
      <li class="fl" v-if="showcamera">
        <i class="iconfont iconxiangji eicotrigger" style="color:#0089FF" @click="camera()"></i>
      </li>
      <li class="fl">
        <i
    ...
    <p class="submit-btn">
      <mt-button
        ...
        :class="isSubmit?'act':''"
        @click="sumint"
      >{{isSubmitEnd?$mt('已提交'):$mt('提交')}}</mt-button>
    </p>
    <input
      ref="uploadPic"
      type="file"
...
export default {
  ...
  methods: {
    camera() {
      Indicator.open(this.$mt("拍照中"));
      this.testkey = new Date().getTime();
      let cb = obj => {
        console.log("评测拍照回调", obj);
        if (obj.code == 0) {
          let res = obj.data;
          ...
          this.isSubmit = true;
          this.pingceData.editimgurl = res.picture;
          console.log("uploadChange", this.pingceData.editimgurl);
        } else {
          Toast(this.$mt("失败"));
        }
    ...
    nativegetgps(cb) {
      ...
      nativecode.jsFn("jsPicture", json);
    },
    moni() {
      let tempobj = {
        data: {
          ...
          data: {
            picture:
              "http://192.168.1.104:8081/downloads/bankefile/20200828/0fdb846339eb2b1d185a855257be95a0.jpg_snap.jpg"
          },
          msg: ""
        },
        key: this.testkey
