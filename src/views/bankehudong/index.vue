<template>
  <div class="hu-dong-container">
    <ul class="list-main">
      ...
      <li class="fontlarge" @click="teacherFn()" v-if="isAPP && !haswxsign">
        <span>{{$mt('上课签到')}}</span>
        <i
          class="iconfont iconqiandao2 eicotrigger bigfont fr"
          :class="eventmsgs.hdTips.sign?'reddot-Tips2':''"
...
export default {
  ...
  computed: {
    ...
    showstuc() {
      ...
      if (nativebridge.platform == "exsoftclass") {
        return false;
      }
      return true;
    },
  ...
  methods: {
    ...
    teacherFn() {
      ...
      this.$router.push({
        path: "/sign",
        query: { bankeid: this.bankeid, caneditbanke: this.isTeacher }
      });
    },
    //评测
    gopingce() {
