<template>
  <div class="Zanbi-main">
    <p style="padding:10px;">{{$mt('设置各类教学活动在学生得分中的占比')}}。</p>
    <div class="content">
      <div class="item-box">
        <p class="tit">{{$mt('资源得分占比')}}：</p>
      ...
      <p class="footer-btn" v-if="caneditbanke">
        <mt-button type="default" @click.native="Submit" class="submit">{{$mt('确认')}}</mt-button>
        <mt-button type="default" @click.native="reSet" class="seSet">{{$mt('恢复默认')}}</mt-button>
      </p>
    </div>
  </div>
</template>
