<template>
  <div class="biggp-memberlist-wrap" ref="memberlistwrap">
    <div class="main">
    ...
    <div
      class="change-state-wrap"
      ref="setsign"
      v-if="showChangeState"
      :style="`left:${curleft}px;top:${curtop}px;`"
    >
      <p class="title tc">{{$mt('设置')}}&nbsp;{{curName}}&nbsp;{{$mt('的签到状态')}}</p>
      <div class="btn-wrap">
        <span
          class
...
export default {
  ...
  created() {},
  mounted() {
    let memberlistwrap = this.$refs.memberlistwrap;
    memberlistwrap.addEventListener("scroll", this.handleScroll, false);
  },
  watch: {},
  methods: {
    handleScroll() {
      if (!this.showChangeState) return;
      let elsetsign = this.$refs.setsign;
      this.p = this.$refs.memberlistwrap.scrollTop;
      if (this.t <= this.p) {
        if (this.$refs.memberlistwrap.scrollTop > elsetsign.offsetTop) {
          this.curscrotop = this.$refs.memberlistwrap.scrollTop;
          this.curtop =
            this.$refs.memberlistwrap.scrollTop + elsetsign.clientHeight;
        }
      } else {
        if (
          this.curscrotop - this.$refs.memberlistwrap.scrollTop >=
          elsetsign.clientHeight
        ) {
          this.curscrotop = this.$refs.memberlistwrap.scrollTop;
          this.curtop =
            this.$refs.memberlistwrap.scrollTop + elsetsign.clientHeight;
    ...
    changeState(e, item, i) {
      ...
      this.curleft = lioffsetLeft;
      this.curtop = eli.offsetTop + 160;
      // this.$nextTick(() => {
