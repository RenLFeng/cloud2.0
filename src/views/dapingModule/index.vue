<template>
  <div class="biggp-group-wrap" ref="doch" v-if="isLoad">
    <div class="fixed-header">
      <HeaderNav :navInfo="navInfo" :starttimedesc="starttimedesc" />
      <!-- <p class="starttimedesc-wrap" v-if="starttimedesc">
        <span class="tit">{{$mt('签到时间')}}:</span>
      ...
      <div class="member-wrap">
        <div class="MemberList">
          <MemberList
            ...
            @setSign="onsetSign"
          />
          <p class="outer" v-if="[].length">{{$mt('旁听学生')}} 6</p>
          <MemberList :members="[]" />
        </div>
      </div>
    </div>
...
import { parseURL, unqitem2 } from "@/util";
...
export default {
  ...
  data() {
    return {
      ...
      timer: null,
      isLoad: false,
      customserver: false, //false 线上环境，true 本地部署环境
      teaurl: ""
    };
  },
  computed: {
  ...
  methods: {
        ...
        if (res.data.code == 0) {
          if (res.data.data.members) {
            ...
            for (let i = 0; i < sm.length; i++) {
              ...
              if (sm[i].name == null) {
                sm[i].name = this.$mt("未知名");
              }
            }
            // this.signMemberList = unqitem2(sm, "memberuserid");
        ...
        .then(res => {
          if (res.data.code == 0) {
            if (res.data.data && res.data.data.signmembers.length) {
              ...
              // this.signMemberList = unqitem2(Data.signmembers,"userid");
    ...
    webqrcode() {
      this.$nextTick(() => {
        ...
        let siddes = `?sid=${this.signdata.id}`;
        if (this.signdata.roomid) {
          siddes = `?roomid=${this.signdata.roomid}`;
        }
        if (this.teaurl) {
          siddes += `&teaurl=${this.teaurl}`;
        }
        let pageurl = `/#/sign${siddes}`;
        let URL = `${location}${pageurl}`;
        console.log("扫码签到路径", URL);
        // QRcode.toCanvas(qcanvas, URL, {}, function(error, url) {
