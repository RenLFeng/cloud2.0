<template>
  <div class="zy-fabu-wrap">
    <mt-header :title="$mt('发布设置')" class="mint-header-f">
      <mt-button slot="left" @click="calce">{{$mt('取消')}}</mt-button>

      <mt-button slot="right" :disabled="isSubmit" @click="publish">{{$mt('开始')}}</mt-button>
    </mt-header>
    <div class="main overflow-scroll main-f">
      <p class="top-tit font16">
        {{$mt('发放对象')}}
        <span class @click="selfn()">{{$mt('全选')}}</span>
      </p>
      ...
      <div class="set-time-wrap">
        <div class="set-time">
          ...
          <mt-cell :title="$mt('允许超时提交作业')" v-if="hassubmittime">
        ...
        <p
          class="tips font-xxs"
          v-if="allowpasstime"
        >{{$mt('开启允许超时提交作业后,老师评分,和指定助教/学生评分类型的作业,系统将允许超时提交作业,但会标记为超时')}}</p>
      </div>
      <!-- <div>
