<template>
  <div class="coures-new-wrap">
    <mt-header :title="cfrom?$mt('编辑课程'):$mt('创建课程')">
      <mt-button slot="left" @click="back">{{$mt('取消')}}</mt-button>

      <mt-button slot="right" @click="onsave" :disabled="savedisable">{{cfrom?$mt('保存'):$mt('创建')}}</mt-button>
    </mt-header>
        ...
        .then(res => {
          ...
          if (res.data.code == 0) {
            ...
            if (tipmsg == "no privilige") {
              ...
              tipmsg = this.$mt('学校课程数已达上限');
            } else if (tipmsg == "over num limit in user") {
              tipmsg = `${this.$mt('您创建的课程数已达上限')}。\r\n${this.$mt('请先结束部分课程')}`;
            }
            Toast(tipmsg);
          }
        })
