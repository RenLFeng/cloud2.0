  ...
  <div class="fontsmall cloudHome" :class="selected=='banke'?'banke':''">
    <div class="mint-header-f mycreate-header" v-if="selected=='banke' && !order">
      <div class="lable tc position-c color1">
        <p class :class="isCreate?'act navbar colord':''" @click="selectClass(1)" v-if="isteacher">
          <span class="tit-name fontnormal position-c nowrap" ref="mycreate">{{$mt(mycreatetext)}}</span>
        </p>
        <p class :class="!isCreate?'act navbar colord':''" @click="selectClass(0)">
          <span class="tit-name fontnormal position-c nowrap" ref="myjoin">{{$mt(myjointext)}}</span>
        </p>
      </div>
      <i class="iconfont iconjiahao position-r fontmaintitle colord" @click="addBankeIcon"></i>
    </div>
    <!-- main -->
    <div class="testdiv" :class="hasnavbar?'noheadercontainer page-wrap cloud':'page-wrap cloud'">
      <mt-tab-container
          ...
          <mt-tab-container-item id="banke">
            ...
            >
              <div class="bankecontainer" :class="!bankeempty?'bankeempty':''">
                ...
                >
                  ...
                  <div class="subclass-wrap" v-if="isCreate">
                    <div
                      ...
                      @click="bankeclick(v)"
                    >
                      <div class="stit color0 font18 ellipse">
                        <p>
                          <span v-if="v.inclass" class="inclass">{{$mt('正在上课')}}</span>
                        </p>
                        <p class="sname">{{v.name}}</p>
                      </div>
                      <p
                      ...
                      >{{$mt('公告')}}:&nbsp;{{v.info}}</p>
                      <div class="sinfo-r">
                        <span class="colora font-xxs ellipse membernum">{{v.membernum}}{{$mt('人')}}</span>
                ...
                />
                <BottomLoadmore
                  v-if="!allLoaded && loading && servestate"
                  showType="loading"
                  loadtext="加载中"
                  type="triple-bounce"
                  color
              ...
              <div v-if="showemptytips" class="tc no-class empty bankeempty-icon">
                <i class="iconfont icontianjia fontmaintitle" @click="addBankeIcon"></i>
                <p v-if="isteacher">{{$mt('暂无课程')}}，{{$mt('点击创建课程或加入班级')}}</p>
                <p v-else>{{$mt('暂无班级')}}，{{$mt('点击加入班级')}}</p>
              </div>
              <!-- <div v-if="!bankeempty&&!isloadtop " class="tc bankeempty-icon">{{bankestatedesc}}</div> -->
            </mt-loadmore>
          </mt-tab-container-item>
        </div>
        <mt-tab-container-item id="myStudy">
          <Mystudy :myStudytext="myStudytext" />
        </mt-tab-container-item>
        <mt-tab-container-item id="mine">
          <MineAbout @changeSelected="onChangeSelected" @clearevnt="onClearevnt"></MineAbout>
        </mt-tab-container-item>
        <mt-tab-container-item id="hudong" v-if="hashudong">
          <Hudong :selected="selected" />
        </mt-tab-container-item>
      </mt-tab-container>
    </div>
    <mt-tabbar v-model="selected" fixed class="cloud cloud-b" v-if="!CliudBar">
      <mt-tab-item id="hudong" @click.native="enterduoping($event)" v-if="hashudong">
        <div class="bankehome hd">
          <!-- <i class="iconfont iconfont-big iconjiaoxue reddot-Tips-wrap"></i> -->
          <!-- <img class="icon-img" src="/assets/hudong/dphdbig.png" alt /> -->
          <div class="img-icon position-c" :class="selected=='hudong'?'act':''">
            <a class="t"></a>
            <a class="b"></a>
          </div>
          <span class="fonttiny  ellipse">{{$mt('多屏互动')}}</span>
        </div>
      </mt-tab-item>
      <mt-tab-item id="banke">
        <div class="bankehome">
          ...
          ></i>
          <span class="fonttiny  ellipse">{{$mt('云班课')}}</span>
        </div>
      </mt-tab-item>
      <mt-tab-item id="myStudy">
        <div class="bankehome">
          <i class="iconfont iconfont-big iconjiaoxue reddot-Tips-wrap"></i>
          <span class="fonttiny  ellipse">{{$mt(myStudytext)}}</span>
        </div>
      </mt-tab-item>
      <mt-tab-item id="mine" @click.native="mineclick">
        <div class="bankehome">
          <i class="iconfont iconfont-big iconwode-xuanzhong"></i>
          <span class="fonttiny ellipse">{{$mt('我的')}}</span>
        </div>
      </mt-tab-item>
      <div class="mark" v-if="showmark" @click="showmarkclick"></div>
    </mt-tabbar>
    <mt-actionsheet :actions="actions" v-model="actionShow" :cancelText="$mt('取消')"></mt-actionsheet>
    <mt-actionsheet :actions="actions2" v-model="actionShow2" :cancelText="$mt('取消')"></mt-actionsheet>
      ...
      <div class="search-wrap">
        ...
        <div class="main" :class="SearchHistoryLen?'act':''">
          ...
          <div v-if="SearchHistoryLen">
            ...
            <p class="tc color9 clear" @click="clearHistory">{{$mt('清除搜索记录')}}</p>
          </div>
    ...
    <p class="totop" ref="totop">{{$mt('没有更多了')}}</p>
  </div>
</template>
...
const Mystudy = () => import("@/views/bankehome/mystudy");
...
export default {
  ...
  computed: {
    showemptytips() {
      if (!this.bankeempty && !this.isloadtop && this.loadEnd) {
        return true;
      }
      return false;
    },
    ...
    selected(newv, ovldv) {
      if (newv == "banke") {
        this.$nextTick(() => {
          ...
          // this.herderdomAddclass();
        });
      } else if (newv == "mine") {
        this.initmine();
      }
      sessionStorage.setItem("setHomeSelected", newv);
    },
    isCreate() {}
  },
  created() {
    const UrlParams = parseURL(window.location.href);
    let selected = sessionStorage.getItem("setHomeSelected");
    if (this.$store.getters.islogin) {
      if (selected) {
      ...
      if (UrlParams.selected) {
        this.selected = UrlParams.selected;
      }
    } else {
      this.selected = "hudong";
    }
    ...
    if (
      ...
      if (bankeScorllState) {
        ...
        this.curpagesetScorllState(bankeScorllState);
      }
  ...
  methods: {
    herderdomAddclass() {
      let mycreate = this.$refs.mycreate;
      let myjoin = this.$refs.myjoin;
      if (mycreate.scrollHeight > mycreate.offsetHeight) {
        this.$refs.mycreate.classList.add("oscroll");
      }
      if (myjoin.scrollHeight > myjoin.offsetHeight) {
        this.$refs.myjoin.classList.add("oscroll");
      }
      mycreate.addEventListener("scroll", this.mycreateScFn, false);
      myjoin.addEventListener("scroll", this.myjoinScFn, false);
    },
    showitem(item) {
    ...
    curpagesetScorllState(localstate) {
      this.page = localstate.page;
      this.allLoaded = localstate.allLoaded;
      this.loading = localstate.loading;
      this.listLoadend = localstate.listLoadend;
    },
    onShowMenu(v) {
      console.log(v);
      this.courseitem = v;
    ...
    //创建or加入
    addBankeIcon() {
      // console.log(this.$mt('付款'));
      // return;
      // this.$http.get('./download.html').then(res=>{
      //     console.log(res.data);
      //      console.log('fdslmk ',$(res.data));
      //      $('.testdiv').html(res.data);

      // })
      // const sprintf = require('sprintf-js').sprintf;
      // let str='a=[% d]=>b=[%d]=>c=[%s]=>d=[%s]';
      // let obj={
      //   aa:55,
      //   bb:88
      // }
      ...
      // //     console.log('成员总数0人==>', this.$mt('成员总数%s人',[c]));
      // //       console.log('成员总数==>', this.$mt('成员总数'));
      //  console.log('现在有0人在0玩==>',this.$mt('现在有%s人在%s玩',[a,b],0));
      // //    console.log('马上到了==>',this.$mt('马上到了',null,1));
      // // console.log(this.$mt('课前',null,1));
      // return;
    ...
    checklogin() {
      ...
      this.$confirm(
        {
          title: "登陆提示",
          msg: `${this.$mt("您未登陆")}，${this.$mt("是否现在登陆")}?`,
          ok: "登陆",
          docb: () => {
    ...
    initbanke() {
      ...
      if (!this.bankeempty) {
        this.bankestatedesc = this.$mt("加载中");
      }
      //! cjy: 防止出错， 总是重新拉取
      this.$http
        .post(url, {
        ...
        .then(res => {
          if (res.data.code == 0) {
            ...
            console.log("all bankes", loadbankes);
            let arrId = [];
          ...
          if (!this.bankeempty) {
            this.bankestatedesc = this.$mt("当前无班课");
          }
        ...
        .catch(res => {
          this.loadEnd = true;
          console.log(res);
          this.bankestatedesc = this.$mt("服务开小差了");
    ...
    filterCurbankeFn(loadbankes, type, first) {
      let tmploadbankes = loadbankes;
      tmploadbankes = this.unqitem(tmploadbankes);
      let temp = [];
      let temp2 = [];
      if (type) {
        temp = tmploadbankes.filter(item => item.userid == this.curuser.id);
    ...
    showmarkclick() {
      this.$confirm({
        ...
        docb: () => {
          this.$router.push("/login");
          sessionStorage.setItem("setHomeSelected", "banke");
        }
      });
    },
  ...
  beforeRouteEnter(to, from, next) {
    next(vm => {
      vm.$nextTick(() => {
        ...
        // let setHomeSelected = sessionStorage.getItem("setHomeSelected") || "";
        // if (setHomeSelected != "null" && setHomeSelected != "") {
        //   vm.selected = setHomeSelected;
        // }
      });
    });
  }
...
.bankehome.hd .img-icon {
  width: 36px;
  height: 40px;
}
.bankehome.hd .img-icon a {
  position: absolute;
  top: 0;
  width: 100%;
  height: 24px;
...
.bankehome.hd .img-icon.act a {
  border: 2px solid #0089ff;
}
.bankehome.hd .img-icon a.t {
  top: -5px;
  left: 5px;
}
.bankehome.hd .img-icon a.b {
  top: 5px;
  right: 5px;
}
.cloudHome {
