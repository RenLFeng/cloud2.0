      ...
      <div class="popupitemlist fontnormal">
        ...
        <ul class="itemlistul">
          <li v-for="(item,sindex) in itemlistdostate" v-bind:key="sindex" class="itemlistli">
            ...
            <div class="listpartdostate">
              <span :class="item.doclass" @click="doJumpItem(sindex)">{{item.dostr}}</span>
            </div>
            <div class="listpartmark">
              <span :class="item.markclass" @click="doJumpItem(sindex)">{{item.markstr}}</span>
            </div>
          </li>
        </ul>
      </div>
...
export default {
  ...
  computed: {
    ...
    curitem: function() {
      ...
      if (
        es.curitemindex < 0 ||
        es.curitemindex >= this.exampaper.itemdetaillist.length
      ) {
        return {};
      }
    ...
    indexdesc: function() {
      var szfmt = "%1$s / %2$s";
      var szret = commontools.sprintf(
        szfmt,
        this.examstate.curitemindex + 1,
        this.exampaper.itemdetaillist.length
      );
      return szret;
    },
    showsubmit: function() {
      if (
        this.examstate.maxitemindex >=
        this.exampaper.itemdetaillist.length - 1
      ) {
        return true;
      }
      return false;
  ...
  methods: {
    ...
    doSubmit: function() {
      ...
      if (this.examstate.state != "do") {
        ...
        if (iundonum > 0) {
          var fmt =
            '还有<span class="twarning">%i题</span>未完成，是否现在交卷？';
