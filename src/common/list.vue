  ...
  >
    ...
    <div class="maincontent" v-if="type=='sign'">
      <div class="mainctitle ellipse">
        {{item.date}}&nbsp;{{$mt(week)}} &nbsp;{{$mt('签到')}}
        <span
          class="fr font-xs color9"
        >{{item.joinnum}} {{$mt('人')}} / {{item.totalnum}} {{$mt('人')}}</span>
      </div>
      <div v-if="classSignId==item.id" class="maincsubtitle colord">{{$mt('正在签到')}}...</div>
    ...
    <div class="maincontent" v-if="type=='pingce'">
      <div class="mainctitle ellipse">
        {{$mt(pingceType)}}
        <span class="fr font-xs color9">{{item.joinnum}}{{$mt('人')}} / {{item.totalnum}} {{$mt('人')}}</span>
      </div>
    ...
    <div class="maincontent pingcedetail" v-if="type=='pingcedetail'">
      ...
      <div v-if="!item.hassubmit">{{$mt('未提交')}}</div>
      <div v-if="ptype!='6' && item.hassubmit" class="maincsubtitle ellipse">{{ptype=='10'?`${$mt('投票给')}:`:`${$mt('提交答案')}:`}} {{$mt(answer)}}</div>
      <div v-if="ptype=='6' && item.hassubmit">{{$mt(item.isResponder)}}</div>
      ...
      <div class="footer" v-if="item.hassubmit">
        ...
        <span class="fr colory font18">{{$mt('得分')}}&nbsp;{{item.score}}</span>
      </div>
    </div>
    <!-- 成员分组 -->
    <div class="maincontent" v-if="type=='group'">
      <div class="mainctitle ellipse">
        {{item.name}}
        <span v-if="index==0" class="fr font-xs colory">{{$mt('默认分组')}}</span>
      </div>
      <div
        class="maincsubtitle color9"
      >{{item.subgroupnum}} {{$mt('人已被划分小组')}}，{{$mt('划分为')}} {{item.subgroupmnum}}{{$mt('个小组')}}{{item.createtime}}</div>
    </div>
    <!-- 收藏记录 -->
    <div class="maincontent common" v-if="type=='common'">
      <div class="mainctitle ellipse">{{item.title}}</div>
      <div class="maincsubtitle color9">
        <span>{{$mt(item.info.typeText)}}</span>
        &nbsp;{{item.info.time}}
      </div>
    </div>
    <i
