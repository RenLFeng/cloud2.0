<template>
    <div class="m-image-crop">
        ...
        <slot></slot>
        <input v-show="filestate==1"
               class="path"
               @change="imageChange"
               ref="path"
               accept="image/*"
               type="file">
        <div v-show="filestate==2"
             ref="fileView"
             class="file-view"
             v-touch:move="move"
             v-touch:end="end"
            ...
            <div class="ctrl">
                <div class="back"
                     ...
                     @click="back"
                     layout="column"
                     layout-align="center center">{{$mt('取消')}}</div>
                <div class="rotate"
                     @click="rotateChange"
                     flex="33"
                     layout="column"
                     layout-align="center center">{{$mt('旋转')}}</div>
                <div class="submit"
                     flex="33"
                     @click="submitFile"
                     layout="column"
                     layout-align="center center">{{$mt('使用')}}</div>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        setCanvas() {
            ...
            let nwidth = this.viewBox.width * sizeScale;
